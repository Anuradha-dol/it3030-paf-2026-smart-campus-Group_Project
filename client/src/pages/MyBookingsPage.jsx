...
// ---------- Helper functions (identical to HomeWithBooking) ----------
...
export default function MyBookingsPage() {
  ...
  const loadBookings = async () => {
    try {
      ...
      applyFilters(allBookings, statusFilter, searchTerm, startDate, endDate);
    } catch (err) {
      console.error(err);
  ...
  const applyFilters = (bookingList, status, search, fromDate, toDate) => {
    ...
    if (search.trim()) {
      ...
      filtered = filtered.filter(b => b.facilityName?.toLowerCase().includes(term));
    }

    // Date range filters
    if (fromDate) {
      filtered = filtered.filter(b => b.bookingDate >= fromDate);
    }
    if (toDate) {
      filtered = filtered.filter(b => b.bookingDate <= toDate);
    }
  ...
  const handleStatusChange = (e) => {
    ...
    applyFilters(bookings, newStatus, searchTerm, startDate, endDate);
  };

  const handleSearchChange = (e) => {
    const term = e.target.value;
    setSearchTerm(term);
    applyFilters(bookings, statusFilter, term, startDate, endDate);
  };

  const handleStartDateChange = (e) => {
    const date = e.target.value;
    setStartDate(date);
    applyFilters(bookings, statusFilter, searchTerm, date, endDate);
  };

  const handleEndDateChange = (e) => {
    const date = e.target.value;
    setEndDate(date);
    applyFilters(bookings, statusFilter, searchTerm, startDate, date);
  };
            ...
            <div className="md-topbar-left">
              ...
              <div className="profile-role-nav-inline">
                {roleNavigationLinks.map((item) => (
                  <Link
                    key={item.to}
                    className={`profile-role-nav-link${item.label === "View Bookings" ? " active" : ""}`}
                    to={item.to}
                  >
          ...
          <div className="md-content-scroll">
            <div className="md-panel">
              ...
              <div className="table-responsive">
                <table className="md-bookings-table">
                  <thead>
                    <tr>
                      <th>ID</th>
                      <th>Facility</th>
